...
launch_role() {
  ...
  case "$agent" in
    claude)
      launch_cmd="export PATH='$SWARM_TOOLS_DIR:$SCRIPT_DIR':\$PATH && cd '$role_worktree' && claude --append-system-prompt-file '$prompt_file' -n 'SwarmForge ${display}' \"\$(cat '$prompt_file')\""
      ;;
    codex)
    ...
    copilot)
      ...
      ;;
    grok)
      launch_cmd="export PATH='$SWARM_TOOLS_DIR:$SCRIPT_DIR':\$PATH && cd '$role_worktree' && grok --cwd '$role_worktree' --rules \"\$(cat '$prompt_file')\""
      ;;
  esac
